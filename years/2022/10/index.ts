...
// solution path: Z:\advent-of-code\years\2022\10\index.ts
// data path    : Z:\advent-of-code\years\2022\10\data.txt
...
async function p2022day10_part1(input: string, ...params: any[]) {
	...
	for (const line of lines) {
		...
		if(command[0] == "noop") {
			...
			commands.push(0, parseInt(command[1]));
		}
	}

	commands.forEach((command, i) => {
		// Inspect during cycle
		if((i + 1) % 40 == 20) {
			sum += x * (i + 1);
		}
...
async function p2022day10_part2(input: string, ...params: any[]) {
	...
	commands.forEach((command, i) => {
		let horizPos = i % width;

		// Inspect during cycle
		if(Math.abs(horizPos - x) <= 1) {
